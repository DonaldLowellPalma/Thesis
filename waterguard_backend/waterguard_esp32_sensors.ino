...
static const float DRINKABLE_TURBIDITY_MAX = 5.0f; // <=5 NTU (ideally <1-2 NTU)
static const float WASH_TURBIDITY_MAX = 30.0f;     // visible cloudiness; still usable for washing
...
#define WIFI_SSID_FALLBACK "Student"
...
#define BACKEND_INGEST_KEY "your-long-random-secret"
...
float calculateNTU(float voltage)
{
    ...
    float ntu = map((long)(voltage * 1000.0f), (long)(vDirty * 1000.0f), (long)(vClear * 1000.0f), 3000, 0);
...
float calculateTDS(float voltage, float temperatureC)
{
    ...
    if (tempForComp < -20.0f || tempForComp > 125.0f)
    {
        tempForComp = 25.0f;
    }
...
float score_Turbidity(float turb)
{
    ...
    if (turb <= 10.0f)
        return 80.0f - (turb - 5.0f) * (30.0f / 5.0f);
    if (turb <= 30.0f)
        return 50.0f - (turb - 10.0f) * (30.0f / 20.0f);
    if (turb <= 50.0f)
        return 20.0f - (turb - 30.0f) * (20.0f / 20.0f);
    return 0.0f;
}
...
void assessWaterQuality(float turbidity, float tds, float ph, float temperature, float &outWQI, String &outCategory, String &outAdvice)
{
    ...
    if (ph < 6.0f || ph > 9.0f || tds > 3000.0f || turbidity > 50.0f || temperature > 40.0f || temperature < 5.0f)
    {
    ...
    const String &advice)
{
    ...
    String url = String(BACKEND_BASE_URL) + BACKEND_INGEST_PATH;
    if (!http.begin(url))
    {
    ...
    if (code == 200 || code == 201)
    {
        Serial.println(F("Backend ingest updated"));
    }
    else
    {
        Serial.print(F("Backend ingest failed: "));
        Serial.println(code);
        String response = http.getString();
        if (response.length() > 0)
        {
